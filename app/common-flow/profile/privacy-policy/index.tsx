...
import { useGetPrivacyPolicyQuery } from '@/store/api/legalApi';
...
const PrivacyPolicyScreen = () => {
  const { data: dbPrivacy, isLoading, isError, refetch } = useGetPrivacyPolicyQuery();
  const { t } = useTranslation();

  const handleShare = async () => {
    try {
      const plainText = dbPrivacy?.data?.description
        ? dbPrivacy.data.description.replace(/<[^>]+>/g, '')
        : 'Privacy Policy details';

      await Share.share({
        title: 'Privacy Policy',
        message: `Özen Et Privacy Policy\n\n${plainText}\n\nFor more information, visit the Özen Et app.`,
      });
    } catch (error) {
  ...
  const formattedDate = dbPrivacy?.data?.updatedAt
    ? new Date(dbPrivacy.data.updatedAt).toLocaleDateString('en-US', {
        month: 'long',
        day: 'numeric',
        year: 'numeric',
      })
      ...
      <LinearGradient
        ...
        <SafeAreaView
          ...
          <View className="flex-row items-center justify-between px-4 pb-3 pt-2">
            <BackButton />
            <Text className="font-nunitoBold text-[18px] text-[#1E293B]">{t('profile.privacyPolicy')}</Text>
          ...
          {/* Error */}
          {(isError || !dbPrivacy?.data) && !isLoading && (
          ...
          {dbPrivacy?.data && !isLoading && (
            <ScrollView
              ...
              <View className="  bg-background p-5 ">{renderHtml(dbPrivacy.data.description)}</View>
